...
&check; Hold each team's current score in a state value
&check; Render each team's current score that is in state to the DOM.
&check; Be able to click the different buttons to increment the appropriate team's score by the correct amount

### STEP 1 & 2 - Adding Team Scores to the Component's State

&check; Import the `useState` hook
&check; Set up the state values for the Lions team score using the state hook
...
- Set up the state value for the Jazz team score using a second state hook call
...
- [&check;]Play around with the styling and make this project your own! Maybe make it a soccer (non-american football) scoreboard, or a rugby or baseball scoreboard
